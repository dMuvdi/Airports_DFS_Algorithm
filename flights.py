...
def number_of_new_connections(starting_airport, airport_codes, connections):
    ...
    # Loop through the adjacency list to see which componets will have indegree 1, this is that 
...
number_of_new_connections(starting_airport, airport_codes, connections)

#Answer: 0

'''
Overall, the time complexity of the entire code is dominated by the steps involving finding SCCs 
and processing the adjacency list, which is typically O(V + E) for the given input. In the worst case, 
it could be O(V * E), but this depends on the specific structure of the airport connections. 
The rest of the code's operations have a lower impact on the overall time complexity.
'''

# Time Complexity: O(V + E) where V in the number of airports and E is the number of routes or connections
